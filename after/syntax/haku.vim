...
syntax keyword hakuCd cd contained
syntax match hakuCdPath "\v\s+.*$" contained
syntax region hakuCdRegion start=/\v[ \t@-]*cd\s+/ end=/\v$/ contains=hakuCd,hakuCdPath

syntax keyword hakuKeyword error pause
" Full list
...
syntax region hakuFor transparent start=/\v\s*for\s/ end=/\v$/ contains=hakuForAndVarStmt,hakuForIn,hakuString,hakuVar,hakuInnerVar,hakuExecString,hakuFunction,hakuNumber
...
syntax region hakuAssign transparent start=/\v\s*[0-9a-zA-Z_-]+\s*\??\=/ end=/\v$/ contains=hakuVar,hakuOperator,hakuString,hakuExecString,hakuAssignVar,hakuFunction,hakuNumber
...
highlight link hakuCd Keyword
highlight link hakuCdPath String
